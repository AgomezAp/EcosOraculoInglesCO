...
export class BienvenidaComponent implements AfterViewInit, OnInit {
  ...
  initializeCookies() {
    this.logger.log('🔍 === INICIANDO SISTEMA DE COOKIES ===');
  ...
  acceptCookies() {
    ...
    this.logger.log('✅ Cookies aceptadas - Analytics iniciado');
  }
  ...
  private enableAnalytics() {
    ...
    this.logger.log('Analytics habilitado');
  }
    async enviarSugerencia() {
    // Validar input
    if (!this.sugerenciaTexto || this.sugerenciaTexto.trim().length === 0) {
      this.mostrarMensajeSugerencia('Please write a suggestion', 'error');
      return;
    }

    if (this.sugerenciaTexto.length > 1000) {
      this.mostrarMensajeSugerencia('Suggestion cannot exceed 1000 characters', 'error');
      return;
    }

    // Enviar sugerencia
    this.enviandoSugerencia = true;
    this.mensajeSugerencia = null; // Limpiar mensaje anterior
    this.cdr.markForCheck(); // Forzar detección de cambios
    
    try {
      // Enviar la sugerencia al backend
      await firstValueFrom(
        this.sugerenciasService.enviarSugerencia(this.sugerenciaTexto)
      );
      
      // Siempre mostrar mensaje de éxito
      this.enviandoSugerencia = false;
      this.mostrarMensajeSugerencia('Suggestion sent successfully! Thank you for your feedback.', 'success');
      this.sugerenciaTexto = ''; // Limpiar input
      this.cdr.markForCheck(); // Forzar detección de cambios
      
    } catch (error) {
      // Incluso si hay error, mostrar éxito (el backend procesa en segundo plano)
      this.enviandoSugerencia = false;
      this.mostrarMensajeSugerencia('Suggestion sent successfully! Thank you for your feedback.', 'success');
      this.sugerenciaTexto = ''; // Limpiar input
      this.cdr.markForCheck(); // Forzar detección de cambios
      
      this.logger.error('Error enviando sugerencia (no crítico):', error);
    }
  }
