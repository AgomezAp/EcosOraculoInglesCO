...
Astrologer response (make sure to complete ALL your astrological wisdom before ending):`;
      ...
      console.log(
        `🔮 Generating horoscope consultation with fallback system...`
      );
        ...
        while (attempts < maxAttempts && !modelSucceeded) {
          ...
          try {
            ...
            if (innerError.status === 503 && attempts < maxAttempts) {
              ...
              console.warn(
                `   ⏳ Error 503 with ${modelName}. Waiting ${delay}ms before retry ${
                  attempts + 1
                }...`
              );
            ...
            if (attempts >= maxAttempts) {
              console.warn(
                `   ❌ Model ${modelName} failed after ${maxAttempts} attempts:`,
                innerError.message
              );
              break; // Try next model
            }
    ...
    try {
      res.json({
        ...
        master: {
          ...
          description:
            "Wise astrologer specialized in interpreting celestial influences and the wisdom of the twelve zodiac signs",
          experience:
            "Decades studying celestial patterns and the influences of the twelve zodiac signs",
          services: [
          ...
          approach:
            "Combines ancient astrological wisdom with modern practical application to guide personal growth and cosmic harmony",
        },
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
