...
exports.validateDreamChatMiddleware = exports.validateChatMiddleware = void 0;
const validateChatMiddleware = (req, res, next) => {
    try {
        const { saintData, userMessage } = req.body;
        // Validar que existan los datos requeridos
        if (!saintData || !userMessage) {
            res.status(400).json({
                success: false,
                error: 'Faltan datos requeridos:  y userMessage son obligatorios',
...
exports.validateChatMiddleware = validateChatMiddleware;
// NUEVO MIDDLEWARE PARA EL INTÉRPRETE DE SUEÑOS
const validateDreamChatMiddleware = (req, res, next) => {
    try {
...
exports.validateDreamChatMiddleware = validateDreamChatMiddleware;
